...
export default (cancelSig: Sig<boolean>, pos: Sig<[number, number]>, stats: () => Perf) => {
    ...
    <div style="background: #000c; color: #fff; font-family: monospace;
      ...
      <div style="position:absolute;top:5px;right:5px;display:flex;gap:.5rem;pointer-events:all">
        ${attrs(
          ev(
            html`<div/>`,
            "mousedown", ((ev: MouseEvent) => dragOffset([ev.clientX - pos()[0], ev.clientY - pos()[1]])),
            "mouseup", () => dragOffset(null)),
        "style", () => "width: 1.5rem; background: repeating-conic-gradient(#808080 0% 25%, transparent 0% 50%) 50% / 2px 2px; cursor: grab" + (dragOffset() ? "bing" : "")
        )}
